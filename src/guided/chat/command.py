...
def chat(model: Optional[str] = typer.Argument(default=None, help="Model name to chat with")):
    ...
    if model is None:
        ...
        if model_cfg is None:
            ...
            rich.print("Run [bold]guide models set-default <name>[/bold] to set a default.")
            raise typer.Exit(1)
    ...
    rich.print(f"[bold]Chatting with[/bold] [cyan]{model_name}[/cyan] via [cyan]{provider.name}[/cyan]")
    rich.print("Type your message and press Enter. Leave blank or press Ctrl+C to exit.\n")
