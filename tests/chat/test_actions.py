...
def test_compact_action_not_enough_messages(capsys):
    ctx = make_ctx()
    ctx.messages.extend([
        {"role": "system", "content": "You are a helpful assistant"},
        {"role": "user", "content": "hello"}
    ])
    result = CompactAction().execute(ctx)
    assert result is False
    assert "Not enough messages to compact" in capsys.readouterr().out


def test_compact_action_executes_successfully(capsys):
    ...
    mock_compressor_instance.compress_prompt.return_value = {
        "compressed_prompt": "Summary result"
    }

    with patch("llmlingua.PromptCompressor", return_value=mock_compressor_instance), \
         patch("torch.cuda.is_available", return_value=True):
        result = CompactAction().execute(ctx)

    assert result is False
    assert "Compacting messages..." in capsys.readouterr().out
    assert len(ctx.messages) == 2
    assert ctx.messages[0]["role"] == "system"
    assert ctx.messages[1]["role"] == "system"
    assert "Previous conversation summary:\nSummary result" in ctx.messages[1]["content"]
    ctx.session.reset_session_id.assert_called_once()


def test_compact_action_fails_without_cuda(capsys):
    from unittest.mock import patch
    ...
    assert result is False
    assert "CUDA is required" in capsys.readouterr().out
